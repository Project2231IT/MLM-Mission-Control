...
// Read single core file
app.get('/api/core-file/:filename', async (req, res) => {
  try {
    const { filename } = req.params;
    if (!CORE_FILES.includes(filename)) {
      return res.status(400).json({ error: 'Invalid core file' });
    }
    const filepath = path.join(WORKSPACE, filename);
    const content = await fs.readFile(filepath, 'utf8');
...
// Write single core file
app.put('/api/core-file/:filename', async (req, res) => {
  try {
    const { filename } = req.params;
    ...
    res.json({ success: true, filename, message: 'File saved' });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`🦀 Mr. Crabs Dashboard running at http://0.0.0.0:${PORT}`);
  console.log(`🌐 Local: http://localhost:${PORT}`);
  console.log(`🌐 LAN: http://172.16.201.40:${PORT}`);
});
